...
/**
 * `compact` squeezes the form to fit inside the knight's shield graphic: tighter
 * spacing, no email field, a shrunken reCAPTCHA. It is a constraint of the
 * shield artwork, not a screen size — the desktop shield needs it and the plain
 * mobile card does not, which is why it is no longer called `mobile`.
 */
function BookingForm({ serviceOptions, section, compact = false }) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className={compact ? 'mt-3 space-y-2' : 'mt-6 space-y-4'}>
        <div>
        ...
        <div>
          ...
          <textarea id="bf-message" name="message" rows={compact ? 2 : 3} placeholder="Briefly describe the issue…" className={`${fieldClass} resize-none mx-auto block !w-[calc(100%-32px)] max-lg:!w-full`} />
        </div>

        <Recaptcha
          ref={recaptchaRef}
          onChange={setRecaptchaToken}
          className={`${compact ? '!-mb-8 [transform:scale(0.58)]' : '-mb-4 [transform:scale(0.92)]'} flex origin-top justify-center`}
        />
        ...
        <div className="relative z-10 mx-auto max-w-[1200px] px-6 pt-10 pb-12 lg:pt-14 lg:pb-24">
          ...
          <div className="grid items-start gap-10 lg:grid-cols-2 lg:gap-12">
            ...
            <div>
              ...
              {/* Mobile-only form. A plain card rather than the shield: the
                  shield silhouette forced a scaled-down, cramped form that read
                  as decoration first and a form second. Ordinary rectangle,
                  ordinary fields, nothing scaled. */}
              <div
                id="quote-form"
                className="mx-auto mb-8 mt-4 block w-[94%] max-w-[460px] scroll-mt-24 rounded-2xl border border-phsInk/10 bg-white p-4 text-phsInk shadow-xl sm:p-6 lg:hidden"
              >
                <BookingForm
